...
void bfs(pair **parent, int x, int y, int des_x, int des_y){
    ...
    while(queue != NULL){
        // printf("%d %d\n", q_tmp->x, q_tmp->y);
        if(ori_map[queue->x][queue->y + 1] != 1 && map_visited[queue->x][queue->y + 1] != 1){
            // debug;
            queue = q_push(queue, queue->x, queue->y + 1); // up
            parent[queue->x][queue->y + 1] = (pair) {queue->x, queue->y};
        }
        if(ori_map[queue->x + 1][queue->y] != 1 && map_visited[queue->x + 1][queue->y] != 1){
            // debug;
            queue = q_push(queue, queue->x + 1, queue->y); // right
            parent[queue->x + 1][queue->y] = (pair) {queue->x, queue->y};
        }
        if(ori_map[queue->x][queue->y - 1] != 1 && map_visited[queue->x][queue->y - 1] != 1){
            // debug;
            queue = q_push(queue, queue->x + 1, queue->y); // down
            parent[queue->x][queue->y - 1] = (pair) {queue->x, queue->y};
        }
        if(ori_map[queue->x - 1][queue->y] != 1 && map_visited[queue->x - 1][queue->y] != 1){
            // debug;
            queue = q_push(queue, queue->x, queue->y - 1); // left
            parent[queue->x - 1][queue->y] = (pair) {queue->x, queue->y};
        }
        
        if(queue->x == des_x && queue->y == des_y){
            break;
        };
        map_visited[queue->x][queue->y] = 1;
        map_visited[x][y] = 1;
        queue = q_pop(queue);
    }
    q_clear(queue);
...
llist *extract_step(pair **parent, llist *steps, int dest_x, int dest_y){
    ...
    while(parent[dest_x][dest_y].x != 0 && parent[dest_x][dest_y].y != 0){
        if(parent[dest_x][dest_y].y < dest_y) steps = push_front(steps, 0); // up
        if(parent[dest_x][dest_y].x < dest_x) steps = push_front(steps, 1); // right
        if(parent[dest_x][dest_y].y > dest_y) steps = push_front(steps, 2); // down
        if(parent[dest_x][dest_y].x > dest_x) steps = push_front(steps, 3); // left
...
void stimulate(llist *steps, int *pos_x, int *pos_y){
    ...
    do{
        // printf("%d\n", tmp->n);
        // printf("%d %d\n", *pos_x, *pos_y);
        if(tmp->n == 0 && ori_map[*pos_x][*pos_y + 1] != 1) (*pos_y)++;
        if(tmp->n == 1 && ori_map[*pos_x + 1][*pos_y] != 1) (*pos_x)++;
        if(tmp->n == 2 && ori_map[*pos_x][*pos_y - 1] != 1) (*pos_y)--;
        if(tmp->n == 3 && ori_map[*pos_x - 1][*pos_y] != 1) (*pos_x)--;
        // debug;
        tmp = tmp->next;
    } while(tmp != NULL);
    // debug;
...
void watch_parent(pair **parent, int x, int y){
    while(parent[x][y].x != 0){
        printf("parent of %d %d : %d %d\n", x, y, parent[x][y].x, parent[x][y].y);
        int tmp_x = x, tmp_y = y;
        x = parent[tmp_x][tmp_y].x;
        y = parent[tmp_x][tmp_y].y;
    }
}
...
int main(){
    ...
    watch_parent(parent, exit_a1, exit_b1);
    debug;
