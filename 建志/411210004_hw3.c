...
int valid(int *path, int ***time, int *QuantumMemory) {
    ...
    if((*time)[0][path[0]] + 1 > QuantumMemory[path[0]] || (*time)[0][path[1]] + 1 > QuantumMemory[path[1]]) return 0;
    for(int i = 2; i < path[Nodes]; i++){
        if((*time)[0][path[0]] + 1 > QuantumMemory[path[0]]\
        || (*time)[0][path[i - 1]] + 2 > QuantumMemory[path[i - 1]]\
        || (*time)[0][path[i]] + 1 > QuantumMemory[path[i]]){
            return 0;
        }
    }
...
int main(){
    ...
    //     for (int i = 0; i < list[k][0]-1; i++) {
    //         for (int j = 0; j < list[k][0]-i-1; j++) {
    ...
    // if(time[0][p_r[0][0]] + 1 > QuantumMemory[p_r[0][0]] || time[0][p_r[0][1]] + 1 > QuantumMemory[p_r[0][1]]) return 0;
    time[0][p_r[0][0]] += 1;
    time[0][p_r[0][1]] += 1;
    printf("%d\n", p_r[0][Nodes]);
    for(int i = 2; i < p_r[0][Nodes]; i++){
        // if(time[0][p_r[0][0]] + 1 > QuantumMemory[p_r[0][0]]\
        // || time[0][p_r[0][i - 1]] + 2 > QuantumMemory[p_r[0][i - 1]]\
        // || time[0][p_r[0][i]] + 1 > QuantumMemory[p_r[0][i]]){
        //     return 0;
        // }
        time[0][p_r[0][0]] + 1 > QuantumMemory[p_r[0][0]];
        time[0][p_r[0][i - 1]] + 2 > QuantumMemory[p_r[0][i - 1]];
        time[0][p_r[0][i]] + 1 > QuantumMemory[p_r[0][i]];
    }
